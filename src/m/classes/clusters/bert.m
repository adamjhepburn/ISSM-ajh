%issmBERT class definition
%
%   Usage:
%      cluster=bert();
%      cluster=bert('np',3);
%      cluster=bert('np',3,'login','username');
...
classdef bert
    properties (SetAccess=public)  
	 ...
	 %}}}
	 methods
		 function cluster=bert(varargin) % {{{

			 %initialize cluster using default settings if provided
			 if (exist('bert_settings')==2), bert_settings; end
		 ...
		 function BuildQueueScript(cluster,dirname,modelname,solution,io_gather,isvalgrind,isgprof,isdakota,isoceancoupling) % {{{
             ...
             fprintf(fid,'module load lmod mpich2/smpd/ge/gcc/64/1.3.2p1 autoconf/2.69 slurm/22.05.2-1\n\n');
		 ...
		 function UploadQueueJob(cluster,modelname,dirname,filelist)% {{{
			 ...
			 disp('Step 1: transferring files to central')
			 central = 'central.aber.ac.uk';
			 path = '/aber/ajh24';
			 file = [dirname '.tar.gz'];
			 %%issmscpout(central,path,cluster.login,cluster.port,{[dirname '.tar.gz']});
			 commandcentral = sprintf('rsync -rav --progress %s %s@%s:%s',...
