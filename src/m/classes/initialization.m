...
classdef initialization
	...
	methods
			...
			if ismember('HydrologyGlaDSAnalysis',analyses),
				if isa(md.hydrology,'hydrologyglads'),
					...
					if md.hydrology.islakes
						...
						if md.hydrology.islakescaled == 1
							md = checkfield(md,'fieldname','initialization.lake_area','NaN',1,'Inf',1,'size',[md.mesh.numberofvertices 1]);
						end
                    end
                end
			end
		...
		function disp(self) % {{{
			...
			fielddisplay(self,'lake_area','initial ice-marginal lake area (for GlaDS with scaled lakes) [m2]');
			...
			fielddisplay(self,'lake_outletQr','sum outlet flux at lake outlet (for GlaDS with lakes) [m3/s]');
			fielddisplay(self,'lake_channelQr','sum channel ONLY flux at lake outlet (for GlaDS with lakes) [m3/s]');
		...
		function savemodeljs(self,fid,modelname) % {{{
			...
			writejs1Darray(fid,[modelname '.initialization.lake_area'],self.lake_area);
